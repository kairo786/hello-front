  ...
  <div overflow=""></div>
      ...
      <div className="relative overflow-hidden">
        ...
        <video
          ...
          preload="auto" // 💡 इसे जोड़ें
          loading="eager" // 💡 इसे जोड़ें
        // poster="/chat-bg.jpg"
        > 
          <source src="/gues.mp4" type="video/mp4" />
      ...
      <motion.section
        ...
        className="relative py-20 bg-linear-to-b from-slate-800/50 to-slate-900/80"
      >
        {/* Retro Grid Pattern */}
        ...
        <div className="container relative z-10 px-4 mx-auto text-center">
          <motion.h2
            className="mb-6 text-6xl font-bold text-transparent md:text-7xl bg-linear-to-r from-cyan-400 via-blue-400 to-purple-400 bg-clip-text"
            initial={{ scale: 0.9 }}
            animate={{ scale: 1 }}
          ...
          <div className="flex flex-col items-center justify-center gap-6 p-6 md:gap-8">
            ...
            {/* Online users badge */}
            {(isSignedIn && isLoaded) &&
              <div className="p-3 font-semibold text-center text-white border-2 border-green-600 shadow-lg bg-linear-to-r rounded-circle w-70 md:w-70 from-green-400 to-emerald-500 md:rounded-full ">
            ...
            <div className="flex flex-col gap-5 md:flex-row md:gap-6">
              <Link href="/chat" passHref>
                <button
                  className={`cursor-pointer relative px-12 py-4 text-lg font-semibold rounded-full shadow-2xl md:mr-4 bg-linear-to-r from-cyan-500 to-blue-600 hover:shadow-cyan-500/25 hover:scale-105 transition-all duration-300 group
                    ${(isLoaded || !isSignedIn)
                      ? ''
                      : 'cursor-not-allowed opacity-70' // Inactive style
                    }`}
                  // disabled={!isLoaded && isSignedIn} // Disable the button when not loaded
                  key="start-chat-button" // Added a unique key for the button
                >
                  <span className="relative z-10">Start Chatting with friends</span>
                  {isLoaded && ( // Only show the hover effect div when loaded
                    <div className="absolute inset-0 transition-opacity duration-300 rounded-full opacity-0 bg-linear-to-r from-blue-600 to-purple-600 group-hover:opacity-100"></div>
                  )}
                </button>
              </Link>

              <Link href="/randomchat" passHref >
                <button
                  // onClick={() => router.push("/randomchat")}
                  className={` relative px-12 py-4 text-lg font-semibold transition-all duration-300 rounded-full shadow-2xl group md:mr-4 bg-linear-to-r from-cyan-500 to-blue-600 hover:shadow-cyan-500/25 hover:scale-105
                  ${(isLoaded || !isSignedIn)
                      ? 'cursor-pointer'
                      : 'cursor-not-allowed opacity-70' // Inactive style
                    }`}
      ...
      {!openbot && (
        ...
        >
          <button
            ...
            className="p-4 transition-all duration-300 rounded-full shadow-2xl bg-linear-to-r from-purple-500 to-pink-500 hover:shadow-purple-500/25"
          >
            <Image src="/robot.gif" unoptimized alt="Chat Assistant" className="w-12 h-12" width={48} height={48} />
          </button>
        </motion.div>
      )}
